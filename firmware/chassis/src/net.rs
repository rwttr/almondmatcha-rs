...
//! duplex, symbol errors - none of which `GenericPhy` exposes, and all of
//! which are the difference between "the link failed" and "here is why".
//! It cannot affect link establishment; see its own doc comment for the
//! delegation rules that guarantee that, and `docs/RUST_REWRITE_PLAN.md`
//! §13.4a for why a full part-specific driver was considered and rejected.
...
/// Number of UDP sockets this board needs: one bound receive socket
/// (`config::SELF_PORT`, inbound `ChassisCommand`) plus one transmit socket
/// each for `ChassisStatus`, `ImuSample`, and `BoardDiagnostics` (via
/// [`tx_socket!`]). Raise this when adding another socket.
...
// independent scalars with no ordering relationship to protect, exactly the
// same reasoning `watchdog.rs`'s `SharedStatus` uses for the same shape.
