...
//! is what fills in every field of that struct on the chassis board.
//!
...
//! result - exactly the shape `main.rs` already uses for a failed IMU init
//! (log it, set a fault/failure bit, keep going). A rover that refuses to
//! start because its PHY didn't answer its identity read is worse than one
//! that drives with a `PostBits::PHY_ID` failure bit set.
...
/// isn't identifiable until the network is up, the motors don't exist until
/// the actuation section of `main` runs, and so on. `main` calls
...
///
/// # Never delays the control loop or the IWDG pet
///
/// This task shares nothing with `watchdog::run`: no motor handle, no IWDG
/// handle, no mutex either could block on. `reset_cause`/`post_run`/
/// `post_pass` are copied in by value at spawn time (all three are `Copy`),
/// and [`crate::net::phy_status`] is a handful of lock-free atomic loads.
/// A slow or failed UDP send here is entirely local to this task's own
/// socket and cannot propagate backpressure into the control loop.
